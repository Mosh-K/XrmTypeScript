...
let funcToIString (desc:bool) (f:Function) =
  ...
  @ [sprintf "%s%s(%s)%s" 
    (if desc then "function " else "") 
    f.name 
    (String.concat ", " (List.collect varToIString f.args)) 
    (someTypeToString f.returnType)]
...
let interfaceToString (i:Interface) =
  ...
  @ indent
    ( List.collect varToIString i.vars
    @ List.collect (funcToIString false) i.funcs
    )
  @ ["}"]

let varsToInlineInterfaceString vars =  
  vars |> List.collect varToIString |> String.concat "; " |> sprintf "{ %s }"


let rec nsToString (m:Namespace) =
  let funcs = 
    match m.ambient with
    | true  -> List.collect (funcToIString true) m.funcs
