...
let internal getCrmServiceClient userName password (orgUrl:Uri) appId returnUrl =
  let mutable orgName = ""
  let mutable region = ""
  ...
  new CrmServiceClient(userName, CrmServiceClient.MakeSecureString(password), region, orgName, false, null, null, appId, Uri(returnUrl), cacheFileLocation, null)
  |> ensureClientIsReady
