module internal DG.XrmTypeScript.InterpretBpfJson
    ...
    | StartsWith "ControlStep" () -> 
      ...
      | _ ->
        ...
        { id = d.controlId
          ...
          displayName = d.dataFieldName
          controlClass = controlClass
          canBeNull = true
          isBpf = false
          targetEntitySets = match box d.entity, controlClass with
...
let interpretBpfs (workflows:Entity[]): Map<string,ControlField list> = 
  ...
  |> Array.map (fun (lname,x) -> 
    ...
    x |> Array.map snd 
    |> List.concat 
    |> List.filter (fun cf -> cf.dataFieldName <> String.Empty)
    |> List.map (fun cf -> {cf with id = sprintf "header_process_%s" cf.dataFieldName; isBpf = true}))
    |> Map.ofArray
