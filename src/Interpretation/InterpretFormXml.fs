module internal DG.XrmTypeScript.InterpretFormXml
...
let getAttribute (enums:Map<string,TsType>) (entity: XrmEntity) (cField: ControlField): XrmFormAttribute option =
  ...
  let attribute =
    entity.attributes
    |> List.tryFind (fun attr -> attr.logicalName = cField.dataFieldName)
  ...
  let aType = 
    if attrType = TsType.Boolean && cField.controlClass = ControlClassId.Picklist then AttributeType.OptionSet TsType.Boolean else
    match cField.controlClass with
    | Picklist 
    | StatusReason  -> AttributeType.OptionSet (enums.TryFind cField.dataFieldName ?| TsType.Number)
    | RadioButtons 
    | CheckBox      -> AttributeType.OptionSet TsType.Boolean
    | MultiPicklist -> AttributeType.MultiSelectOptionSet (enums.TryFind cField.dataFieldName ?| TsType.Number)
    ...
    | PartyListLookup 
    | RegardingLookup 
    | Lookup        -> AttributeType.Lookup (getTargetEntities cField.targetEntitySets attribute)
    ...
    | _             ->  match attribute with
                        ...
                        | Some a -> match a.specialType with
                                    ...
                                    | SpecialType.EntityReference -> AttributeType.Lookup (getTargetEntities cField.targetEntitySets attribute)
...
let getControl  (enums:Map<string,TsType>) (entity: XrmEntity) (cField:ControlField): XrmFormControl option =
  ...
  let formAttr = getAttribute enums entity cField
  ...
  let cType = 
    ...
    | PartyListLookup 
    | RegardingLookup 
    | Lookup                -> ControlType.Lookup (getTargetEntities cField.targetEntitySets attribute)
    ...
    | _     ->  match formAttr with
                | Some (_, _, (AttributeType.Lookup _), _)                 -> ControlType.Lookup (getTargetEntities cField.targetEntitySets attribute)
                | Some (_, _, (AttributeType.OptionSet _), _)              -> ControlType.OptionSet
                | Some (_, _, (AttributeType.Number), _)                   -> ControlType.Number
                | Some (_, _, (AttributeType.Date), _)                     -> ControlType.Date
...
let getDescription (xEl:XElement) =
  let labels = xEl.Descendants(XName.Get "labels") |> Seq.head
  let label = labels.Descendants(XName.Get "label") |> Seq.head 
  getValue label "description"
...
let renameControls (controls: XrmFormControl list) : XrmFormControl list =
    let findUniqueName (used: Set<string>) (baseName: string) (isBpf: bool) =
        // Recursive helper to find the first available suffix
        let rec generate i =
            let candidate = 
                if i = 0 then baseName 
                else if isBpf then sprintf "%s_%d" baseName i 
                else sprintf "%s%d" baseName i
            
            if not (used.Contains candidate) then candidate
            else generate (i + 1)
        generate 0

    // Use List.fold to carry the Set of used names and the building list of controls
    let _, renamedControls = 
        ((Set.empty, []), controls) 
        ||> List.fold (fun (usedNames, acc) (name, cm, a, ct, isBpf, canBeNull) ->
            let uniqueName = findUniqueName usedNames name isBpf
            let newControl = uniqueName, cm, a, ct, isBpf, canBeNull
            
            // Return updated set and prepend the new control to the accumulator
            usedNames.Add uniqueName, newControl :: acc
        )

    // Reverse at the end because we prepended for performance
    List.rev renamedControls

let getCompositeField (cFiled: ControlField) subFieldName ty : ControlField =
    { cFiled with
        id = sprintf "%s_compositionLinkControl_%s" cFiled.id subFieldName
        dataFieldName = subFieldName
        controlClass = ty
...
let getCompositeFields : ControlField list -> ControlField list =
  List.choose (fun field  ->
    let { dataFieldName = datafieldname } = field
...
let getQuickViewFormControls : ControlField list -> XrmFormQuickViewForm list =
  List.choose (fun cField ->
    match cField.quickViewForms with
    | Some qf ->
      ...
      Some (cField.id, cField.displayName, ref)
    | None -> None
  )
...
let getControlFields (entity: XrmEntity) (form: XElement) : ControlField list =
    ...
    |> Seq.map (fun cell -> 
      ...
      let uniqueId = getValue ctrl "uniqueid"
      ...
      let targetEntities = 
        ...
        if Seq.isEmpty parms then
          ...
          |> List.collect (fun r -> if r.schemaName = rel then r.relatedInfo |> List.map (fun e -> e.EntitySetName) else [])
        else
          parms
      ...
      let baseControl = {
         ...
         displayName = getDescription cell; 
...
let interpretFormXml (enums:Map<string,TsType>) (bpfFields: ControlField list option) entity (systemForm:Entity) =
    ...
    |> Seq.choose (fun tab -> 
      ...
      let sections: XrmFormSection list = 
        ...
        |> Seq.choose (fun section -> 
          let sectionName = getValue section "name"
          let sectionHandle = if not (String.IsNullOrEmpty sectionName) then sectionName else getValue section "id"
          if sectionHandle = null then None
          else
  ...
  let stateAttr =
    if
        not (controlFields |> List.exists (fun cf -> cf.dataFieldName = "statecode"))
        && entity.attributes |> List.exists (fun a -> a.logicalName = "statecode")
    then
        [ { id = "statecode"
            dataFieldName = "statecode"
            displayName = "statecode"
            controlClass = ControlClassId.Picklist
            canBeNull = false
            isBpf = false
            targetEntitySets = None
            quickViewForms = None } ]
  ...
  { XrmForm.name =  name |> Utility.sanitizeString
    ...
    attributes = 
      controlFields @ compositeFields @ bpfFields @ stateAttr
      |> List.choose (getAttribute enums entity)
      |> List.distinctBy (fun (a, _, _, _) -> a)
      |> List.sortBy (fun (a, _, _, _) -> a)
