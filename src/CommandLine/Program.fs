...
let loadGen parsedArgs =
  ...
  XrmTypeScript.GenerateFromFile(
    getGenerationSettings parsedArgs,
    filename)

/// Load metadata from file and save to another file
let loadSaveFile parsedArgs =
  let loadFile =
    match Map.tryFind "load" parsedArgs with
    | Some p -> p
    | None -> failwithf "No load argument found"
  ...
  let saveFile =
    ...
    | Some p -> p
    | None -> failwithf "No save argument found"

  XrmTypeScript.SaveMetadataFromFile(loadFile, saveFile)
...
let dataSave parsedArgs =
  ...
  XrmTypeScript.SaveMetadataToFile(
...
let executeWithArgs argv =
  ...
  let hasSave = parsedArgs |> Map.containsKey "save"
  let hasLoad = parsedArgs |> Map.containsKey "load"
  ...
  | _ when hasSave && hasLoad ->
    parsedArgs |> loadSaveFile
