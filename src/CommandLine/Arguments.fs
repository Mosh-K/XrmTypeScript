...
type Args private () =
  ...
  static member generationArgs = [
    ...
    { command="web"
      altCommands=["w"]
      description="Set to true if Web entity types should be generated."
      required=false }

    { command="skipXrmApi"
      altCommands=["sxa"]
      description="Set to true to skip generation of Xrm.WebApi overloads."
      required=false }
  ...
  static member fullArgList = List.concat [ Args.connectionArgs; Args.generationArgs |> Args.removeRequired; Args.flagArgs; [Args.useConfig] ]
